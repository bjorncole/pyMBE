...
def test_association_labels(load_library):
    ...
    links_ns = [library_model_ns
               for library_model_ns in load_library.ownedElement
               if library_model_ns.throughOwningMembership[0].declaredName == 'Links'][0]
    ...
    assert "BinaryLink «Association» ([Anything «Classifier»] ←→ [Anything «Classifier»])" in link_assoc_names
    assert "SelfLink «Association» ([Anything «Classifier»] ←→ [Anything «Classifier»])" in link_assoc_names
...
def test_inherited_name_labels(load_library):
    ...
    vvals_ns = [library_model_ns
               for library_model_ns in load_library.ownedElement
               if library_model_ns.throughOwningMembership[0].declaredName == 'VectorValues'][0]
