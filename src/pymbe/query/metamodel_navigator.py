...
def get_lower_multiplicty(type_ele):
    ...
    if not 'throughOwningMembership' in type_ele._derived:
        return lower_mult
    multiplicity_ranges = [mr for mr in type_ele.throughOwningMembership if mr['@type'] == 'MultiplicityRange']
    if len(multiplicity_ranges) == 1:
        literal_value = [li.value for li in multiplicity_ranges[0].throughOwningMembership if li['@type'] == 'LiteralInteger']
...
def get_upper_multiplicty(type_ele):
    ...
    if len(multiplicity_ranges) == 1:
        ...
        literal_value = [li.value for li in multiplicity_ranges[1].throughOwningMembership if li['@type'] == 'LiteralInteger']
